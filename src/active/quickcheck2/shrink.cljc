(ns active.quickcheck2.shrink
    (:require [active.quickcheck2.tree :refer :all]))

(defn dont-shrink
  "takes the root of an shrinking tree as tree"
  [tree-with-shrink]
  (tree tree-outcome (tree-outcome tree-with-shrink)
        tree-shrinks []))
...
(defn sequence-shrink
  ...
  (tree tree-outcome (map tree-outcome seq-of-trees)
        tree-shrinks (map (partial sequence-shrink merge-shrinking-options-f)
                          (merge-shrinking-options-f seq-of-trees))))
